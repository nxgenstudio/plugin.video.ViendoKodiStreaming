        ...
        for m in data:
            if m and m[0] != '#':
                ...
                if index != -1:
                    ...
                    index = value.find('|')
                    if value[:index] == 'viendokodi.streaming.locale':
                        value = common.translate(int(value[index+1:]))
                    elif value[:index] == 'viendokodi.streaming.image':
...
# ----------------------------------------------------------
    def add(self, rootFolder=None):
        menuItems = ["Add folder", "Add ViendoKodiStreaming item", "Add xbmc favourite"]
        ...
        if select == 0:
            ...
            common.showInfo('Please browse through ViendoKodiStreaming and use \ncontext menu entry "Add to ViendoKodiStreaming favourites"')
        elif select == 2:
