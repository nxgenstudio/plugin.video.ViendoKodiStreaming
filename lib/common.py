...
__settings__ = xbmcaddon.Addon(id='plugin.video.ViendoKodiStreaming')
__icon__ = xbmcaddon.Addon(id='plugin.video.ViendoKodiStreaming').getAddonInfo('icon')
translate = __settings__.getLocalizedString
enable_debug = True
language = xbmc.getLanguage

def log(msg, level=xbmc.LOGDEBUG):
    plugin = "ViendoKodiStreaming"
...
class Paths:
    ...
    imgDir = os.path.join(resDir, 'imagenes')
    modulesDir = os.path.join(resDir, 'modulos')
    catchersDir = os.path.join(resDir,'catchers')
    dictsDir = os.path.join(resDir,'diccionarios')
